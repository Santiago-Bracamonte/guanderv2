...
function DetailRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-col gap-0.5">
      <span className="text-[11px] font-semibold uppercase tracking-wide" style={{ color: 'var(--guander-muted)' }}>{label}</span>
      <span className="text-sm font-medium" style={{ color: 'var(--guander-ink)' }}>{value || '—'}</span>
    </div>
  );
}
...
export default function UsuariosClient({ initialUsers, totalUsers: initialTotal }: { initialUsers: UserItem[]; totalUsers: number }) {
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      if (!res.ok) { alert('Error al actualizar usuario'); return; }
      setUsers((prev) => prev.map((u) =>
        u.id_user === editUser.id_user
          ? { ...u, name: editName, last_name: editLastName, email: editEmail, tel: editTel, state: editState }
          : u
      ));
      setEditUser(null);
    } catch { alert('Error de red'); }
    ...
    <div className="space-y-6">
          ...
          <table className="w-full text-sm min-w-[600px]">
            <thead>
              <tr style={{ backgroundColor: 'var(--guander-mint)' }}>
                ...
                <th className="text-left px-5 py-3 font-semibold" style={{ color: 'var(--guander-forest)' }}>Rol</th>
                <th className="text-left px-5 py-3 font-semibold" style={{ color: 'var(--guander-forest)' }}>Estado</th>
                <th className="text-left px-5 py-3 font-semibold" style={{ color: 'var(--guander-forest)' }}>Registro</th>
            ...
            <tbody>
              ...
              {filtered.map((user) => (
                <tr key={user.id_user} className="border-t" style={{ borderColor: 'var(--guander-border)' }}>
      ...
      <Modal open={!!viewUser} onClose={() => setViewUser(null)}>
        <div className="p-6 pb-3 flex items-center justify-between">
          <h2 className="text-lg font-bold" style={{ color: 'var(--guander-ink)' }}>Detalle de Usuario</h2>
          <button onClick={() => setViewUser(null)} className="w-8 h-8 rounded-full flex items-center justify-center cursor-pointer hover:bg-gray-100 transition"><X size={16} /></button>
        </div>
        {viewUser && (
          <div className="px-6 pb-6 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <DetailRow label="Estado" value={viewUser.state === 1 ? 'Activo' : 'Inactivo'} />
              <DetailRow label="Fecha de registro" value={viewUser.date_reg?.split('T')[0] ?? '—'} />
            </div>
            <button onClick={() => setViewUser(null)}
              ...
              style={{ backgroundColor: '#c5cdb3', color: '#3d4f35' }}>Cerrar</button>
          </div>
        )}
      </Modal>
      ...
      <Modal open={!!editUser} onClose={() => setEditUser(null)}>
        <div className="p-6 pb-3 flex items-center justify-between">
          <h2 className="text-lg font-bold" style={{ color: 'var(--guander-ink)' }}>Editar Usuario</h2>
          <button onClick={() => setEditUser(null)} className="w-8 h-8 rounded-full flex items-center justify-center cursor-pointer hover:bg-gray-100 transition"><X size={16} /></button>
        </div>
        ...
        <div className="space-y-4 px-6 pb-6">
          ...
          <div className="flex gap-3 pt-2">
            <button onClick={() => setShowAdd(false)}
              className="flex-1 py-2.5 rounded-xl text-sm font-semibold transition cursor-pointer hover:opacity-90"
              style={{ backgroundColor: '#c5cdb3', color: '#3d4f35' }}>Cancelar</button>
            <button onClick={handleAdd} disabled={saving || !formName.trim() || !formEmail.trim() || !formUsername.trim()}
              className="flex-1 py-2.5 rounded-xl text-sm font-semibold text-white transition cursor-pointer hover:opacity-90 disabled:opacity-50"
              style={{ backgroundColor: 'var(--guander-forest)' }}>
